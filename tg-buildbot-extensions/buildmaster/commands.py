...
from winunix import correctseperator
...
class SlaveEasyInstall(SlaveShellCommand):
    def start(self):
        ...
        env['PATH'] = correctseperator(args['path']) 
        if OS.path.seperator == '\\':
        ...
        commandline = ["easy_install","--prefix",".",args['package']]
...
class SlaveBuildEggEE(SlaveShellCommand):
    def start(self):
        ...
        commandline = ["easy_install","--prefix",".","-zmxd",".",args['package']]
        ...
        commandline = ["easy_install","--prefix",".","-H","None","-f",".",args['package']]
        c = ShellCommand(self.builder, commandline,
                         workdir, environ=env,
                         timeout=args.get('timeout', None),
                         sendStdout=args.get('want_stdout', True),
...
class SlavePython(SlaveShellCommand):
    def start(self):
        ...
        newworkdir = correctseperator(args['workdir'])
        workdir = os.path.join(self.builder.basedir, newworkdir)
        script = correctseperator(script)
