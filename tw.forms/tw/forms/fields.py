...
class FormField(InputWidget):
    ...
    def _get_engine_name(self):
        if self._engine_name is None:
            dv = tw.framework.default_view
            return dv not in available_engines and 'genshi' or dv
        return self._engine_name
    def _set_engine_name(self, name):
        self._engine_name = name
    engine_name = property(_get_engine_name, _set_engine_name, doc="""\
        The engine name will default to the default_view configured in the
        middleware if templates are available for that engine.
        If no templates are available then it will default to use Genshi for
...
class TableMixin:
    ...
    """
    params = ["table_attrs", "show_labels"]
    table_attrs = {}
    show_labels = True
...
class CheckBoxTable(CheckBoxList):
    ...
    def update_params(self, d):
        ...
        opts = list(d.options)
        n_opts = len(opts)
        d.num_rows = int(math.ceil(n_opts / (d.num_cols * 1.0)))
        mod = n_opts % d.num_cols
        if mod:
            d.blank_cells = d.num_cols - mod
        else:
            d.blank_cells = 0
