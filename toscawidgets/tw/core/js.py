...
class TWEncoder(simplejson.encoder.JSONEncoder):
    ...
    Example::
        ...
        >>> encode({'onLoad': js_function("do_something")(js_symbol("this"))})
    ...
    def _iterencode_default(self, obj, markers):
        if isinstance(obj, self.pass_through):
            ...
            yield simplejson.encoder.encode_basestring(obj.id)
        else:
            assert not isinstance(obj, self.pass_through),  `type(obj)`
            for i in super(TWEncoder, self)._iterencode_default(obj, markers):
                yield i
