...
__connection__ = hub = PackageHub('bbwi')
    ...
    def set_parameter(self,parameter):
        ...
        for k,v in parameter.iteritems():
            if v != "":
                temp[k] = v
        self.parameter = temp
    def get_parameter_as_kv_pairs(self):
        temp = ""
        for k,v in self.parameter.iteritems():
            temp = temp + "%s=%s " %(k,v)
        return temp ;
...
class BuildTask:
	...
	self.name = name
    def add_step(self,buildstep):
        self.build_steps.append(buildstep)
        
class BuildSteps(SQLObject):
    name = StringCol(length=40,alternateID=True)
    parameter = StringCol(length=200)
    
    def getparameterlist(self):
        return self.parameter.split(' ')

""" This table contains the names of all known build task functions"""    
class BuildTaskFuncs(BuildSteps):
    pass
