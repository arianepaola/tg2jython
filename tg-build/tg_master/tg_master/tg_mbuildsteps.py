...
class BaseStep(LoggingBuildStep):
    ...
    parms = BuildStep.parms + ['description', 'descriptionDone',
                               'env', 'workdir']
...
class VirtualEnv(BaseStep):
    """
    Creates a virtual environment in workdir. This VirtualEnv has no global
    site packages
    """
...
class BuildDocEpy(BaseStep):
    """
    Builds documentation from files contained in doc_dir. 
    This step uses epydoc.
    
    :keyword doc_dir: directory which contains documentation files
    :type doc_dir: string
    :keyword needed_conf_file: path to a needed config file that has
      to be downloaded from the build master
    :type needed_conf_file: string
    """
    
    parms = BaseStep.parms +['doc_dir', 'needed_conf_file']
    def __init__(self, *args, **kwargs):
        BaseStep.__init__(self, slave_name="BuildDocEpy", *args, **kwargs)
        self.description = "Starting documentation"
        self.descriptionDone = "Documentation finished"

class BuildDocSphinx(BaseStep):
    """
    Builds documentation from files contained in doc_dir.
    This step uses sphinx.
    
    :keyword doc_dir: directory which contains documentation files
    :type doc_dir: string
    :keyword needed_conf_file: path to a needed config file that has to be 
      downloaded from the build master
    :type needed_conf_file: string
    """
    parms = BaseStep.parms +['doc_dir', 'needed_conf_file']
    def __init__(self, *args, **kwargs):
        BaseStep.__init__(self, slave_name="BuildDocSphinx", *args, **kwargs)
        self.description = "Starting documentation"
        self.descriptionDone = "Documentation finished"